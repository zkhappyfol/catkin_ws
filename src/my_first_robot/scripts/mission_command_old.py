    ...
    def parse_with_llm(self, command_text):
        ...
        try:
            ...
            if response_json.get("intent") == "find_object" and response_json.get("color") in self.color_ranges:
                return response_json.get("color")
        except Exception as e:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.robot_state == "IDLE":
                # 在IDLE状态，我们尝试去听指令
                # (为了简化测试，我们先手动设置一个目标)
                #rospy.loginfo_once("当前为手动测试模式，自动设置寻找目标为'green'...")
                #self.current_target_color = "green" # 手动设置目标
                text = self.listen_and_recognize()
                if text:
                    # 【逻辑修正】解析出的颜色，要赋值给self.current_target_color
                    target_color = self.parse_with_llm(text)
                    if target_color:
                        self.current_target_color = target_color
                        self.robot_state = "SEARCHING"
                        self.last_error_x = 0 # 重置PD控制器
